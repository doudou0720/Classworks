    ...
    <v-list>
      ...
      <template v-if="currentProvider === 'kv-local'">
        <v-list-item>
          ...
          <template #append>
            <v-btn color="error" variant="tonal" @click="confirmClearIndexedDB"> 清除 </v-btn>
          </template>
        </v-list-item>
        <v-list-item>
          <template #prepend>
      ...
      try {
        ...
        if (isRotationEnabled()) {
          ...
          if (triedServers.length > 1) {
            const serverList = triedServers
              .map((s, i) => `${i + 1}. ${s.url} (${s.status === "success" ? "成功" : "失败"})`)
              .join("\n");
            message += `\n\n依次尝试的服务器:\n${serverList}`;
        ...
        if (triedServers.length > 0) {
          const serverList = triedServers
            .map((s, i) => `${i + 1}. ${s.url} (失败${s.error ? `: ${s.error}` : ""})`)
            .join("\n");
          errorMessage += `\n\n依次尝试的服务器:\n${serverList}\n\n所有服务器均连接失败`;
        }

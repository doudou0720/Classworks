    ...
    <v-card class="rate-limit-modal">
      ...
      <v-card-text class="pa-6">
        ...
        <v-card v-if="activeRequests.length > 0" class="mb-4" flat>
          <v-card-text>
            <v-list v-for="(request, index) in activeRequests" :key="index" class="mb-4">
              <v-list-item color="primary" prepend-icon="mdi-web">
                <v-list-item-title>
                  等待时间:
                  <span class="text-primary font-weight-bold">{{ request.remainingSeconds }}</span>
                </v-list-item-title>
            ...
            <v-divider v-if="index < activeRequests.length - 1" class="my-3" />
          </v-card-text>
        </v-card>

        <div class="text-body-2 text-grey">请在等待时间后再次尝试，或减少请求频率以避免限制。</div>
      </v-card-text>
...
const RateLimitModalComponent = {
  ...
  methods: {
    ...
    show(resetTime, path, method) {
      ...
      const remainingSeconds = Math.max(0, Math.floor((new Date(resetTime) - new Date()) / 1000));

    ...
    <v-card-text class="pa-8">
      ...
      <div v-show="currentStep === 1" class="step-content">
        <div class="text-center mb-6">
          <v-icon class="mb-4" color="primary" size="80"> mdi-hand-wave </v-icon>
          <h2 class="text-h4 mb-3">欢迎使用 Classworks</h2>
          <p class="text-body-1 text-medium-emphasis">适用于班级大屏的作业板小工具</p>
        </div>
      </div>

      <!-- 步骤 2: Classworks 和 KV 的关系图 -->
      <div v-show="currentStep === 2" class="step-content">
        <h3 class="text-h5 mb-6 text-center">Classworks 和 Classworks KV 的关系</h3>

        <v-card class="pa-6 mb-6" color="primary" variant="tonal">
          <div class="relationship-diagram">
            <!-- Classworks 应用 -->
            <div class="diagram-item">
              <v-card class="pa-4" color="blue-darken-1" elevation="8">
                <div class="text-center">
                  <v-icon color="white" size="60"> mdi-laptop </v-icon>
                  <h4 class="text-h6 text-white mt-2">Classworks</h4>
                  <p class="text-caption text-white mt-1">作业板应用</p>
                </div>
              </v-card>

              <div class="diagram-description mt-3">
                <v-chip class="mb-2" color="blue" size="small" variant="flat"> 前端应用 </v-chip>
            ...
            <div class="diagram-connector">
              <v-icon color="primary" size="40"> mdi-swap-horizontal </v-icon>
              <div class="text-caption font-weight-bold mt-2">数据同步</div>
            </div>

            <!-- Classworks KV -->
            <div class="diagram-item">
              <v-card class="pa-4" color="green-darken-1" elevation="8">
                <div class="text-center">
                  <v-icon color="white" size="60"> mdi-cloud-sync </v-icon>
                  <h4 class="text-h6 text-white mt-2">Classworks KV</h4>
                  <p class="text-caption text-white mt-1">云端数据库</p>
                </div>
              </v-card>
      ...
      <div v-show="currentStep === 3" class="step-content">
        <h3 class="text-h5 mb-6 text-center">你需要在多个设备上查看作业吗？</h3>

        <v-card class="mb-6 pa-4" color="info" variant="tonal">
          <div class="text-body-2">比如：在家里电脑、手机上查看，或者多个教室设备共享数据</div>
        </v-card>
        ...
        <div class="button-group">
          ...
          >
            <div class="d-flex flex-column align-center py-2">
              <v-icon class="mb-2" size="40"> mdi-cloud-check </v-icon>
          ...
          >
            <div class="d-flex flex-column align-center py-2">
              <v-icon class="mb-2" size="40"> mdi-laptop </v-icon>
      ...
      <div v-show="currentStep === 4 && storageType === 'cloud'" class="step-content">
        <div class="text-center mb-6">
          <v-icon class="mb-4" color="primary" size="80"> mdi-cloud-cog </v-icon>
          <h3 class="text-h5 mb-4">需要先设置云端账号</h3>
        </div>

        <v-card class="pa-6 mb-6" variant="tonal">
          <div class="d-flex flex-column flex-sm-row align-center">
            <div class="flex-grow-1">
              <h4 class="text-h6 font-weight-bold mb-2">自动注册设备</h4>
        ...
        <v-card
          ...
          @click="openKVSite"
        >
          <v-icon class="mb-3" size="48"> mdi-open-in-new </v-icon>
          ...
          <v-expansion-panel>
            <v-expansion-panel-title>
              <div class="d-flex align-center">
                <v-icon class="mr-3" color="warning"> mdi-help-circle </v-icon>
          ...
          <v-expansion-panel>
            ...
            <v-expansion-panel-text>
              <v-card class="pa-4" color="info" variant="tonal">
                ...
                <div class="text-body-2 mb-2">例如：</div>
                <ul class="text-body-2 ml-4">
                  <li class="mb-1">班级大屏使用一个密码</li>
                  <li class="mb-1">教师设备使用另一个密码</li>
                  <li>学生设备使用不同的密码</li>
                </ul>
                <div class="text-body-2 mt-3">请联系您的管理员获取对应设备类型的密码。</div>
              </v-card>
            </v-expansion-panel-text>
          </v-expansion-panel>
        </v-expansion-panels>
      ...
      <div v-show="currentStep === 5" class="step-content">
        <div class="text-center mb-6">
          ...
          <h3 class="text-h5 font-weight-bold mb-2">渐进式注册</h3>
          <p class="text-body-2 text-medium-emphasis">您可以暂时不配置 Classworks KV</p>
        </div>
        ...
        <v-row>
          <v-col cols="12">
            <v-card :color="statusColor" variant="tonal">
              <v-card-item>
                ...
                <div v-if="deviceInfo" class="text-body-2 mb-2">
                ...
                <div v-if="progressiveStatus === 'error'" class="text-body-2 text-error">
                  {{ progressiveError }}
                </div>
              </v-card-item>
            </v-card>
          ...
          <v-col cols="12">
            <v-card variant="outlined">
              <v-card-item>
                <div class="text-subtitle-2 font-weight-medium mb-3">过程日志</div>
                <div class="log-box">
                  <div v-for="(log, i) in logs" :key="i" class="text-caption log-line">
                    {{ log.time }} · {{ log.message }}
                  </div>
                  <div v-if="!logs.length" class="text-caption text-medium-emphasis">等待开始…</div>
                </div>
              </v-card-item>
            </v-card>
          </v-col>
